...
class BuyerController:
    ...
    def search_listings(self):
        ...
        if request.method == 'POST':
            ...
            cars = self.buyer_entity.search_listings(search_term)
        else:
            # If no search term, retrieve all listings
            cars = self.buyer_entity.search_listings()
    ...
    def view_listing(self, car_id):
        ...
        car = self.buyer_entity.get_car_by_id(car_id)
    ...
    def loan_calculator(self):
        ...
        cars = self.buyer_entity.get_all_cars()
        monthly_payment = None

        if request.method == 'POST':
            # Extract loan details from the form
            car_id = int(request.form['car_id'])
            selected_car = self.buyer_entity.get_car_by_id(car_id)
